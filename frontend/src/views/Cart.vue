...
const total = computed(() => {
  return cartItems.value.reduce((sum, item) => {
    return sum + (item.seckillPrice || item.goodsPrice) * item.quantity
  }, 0)
})
...
const cartColumns = [
  {
    ...
    render(row) {
      return h('div', { style: { display: 'flex', alignItems: 'center', gap: '12px' } }, [
        h('div', { class: `cart-thumb g${(row.id % 6) + 1}` }, row.goodsName?.charAt(0)),
        h('div', {}, [
          h('p', { style: { margin: 0, fontWeight: '600' } }, row.goodsName),
          h('p', { style: { margin: '4px 0 0', color: '#999', fontSize: '12px' } }, row.goodsTitle),
        ])
      ])
    }
  },
  ...
  },
  {
    ...
    render(row, index) {
      return h(NInputNumber, {
        ...
        onUpdateValue: (v) => { cartItems.value[index].quantity = v }
      })
    }
  },
  {
    ...
    render(row) {
      const price = row.seckillPrice || row.goodsPrice
      const subtotal = (price * row.quantity).toFixed(2)
      return h('span', {
  ...
  },
  {
    ...
    render(row, index) {
      return h(NButton, {
        ...
        onClick: () => cartItems.value.splice(index, 1)
      }, { default: () => '删除' })
    }
  },
]
